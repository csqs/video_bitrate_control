...
LOG_FILE = './test_results/log_netprobe_rl'
NN_MODEL = './models/nn_model_ep_3200.ckpt'

# LOG_FILE = './results/log_train_rl'
# NN_MODEL  = './models/nn_model_ep_178800.ckpt'
# LOG_FILE = './results/log_train_wang14w_rl'
# NN_MODEL  = './models/nn_model_ep_143600.ckpt'
...
def main():
    ...
    log_path = LOG_FILE + '_' + str(net_env.netbw)+'_'+str(net_env.net_lossrate)+'_'+str(net_env.videochat_time)
    ...
    with tf.Session() as sess:
	    ...
	    x_funtion_top = (bitrate_send_last_probe * (1 - lossrate_recv_last_probe) - VIDEO_BIT_RATE[bit_rate])  / M_IN_K
            ...
            index_array = np.argmax(action_prob[0])
            bit_rate = np.min(index_array)
            ...
            if end_of_video:
                ...
                if end_of_validation:
                    ...
                    print  'all avg reward_mean: ', reward_mean_cur
                    break
                # print 'file_name: ', all_file_names[net_env.trace_idx]
                log_path = LOG_FILE + '_' + str(net_env.netbw) + '_' + str(net_env.net_lossrate) + '_' + str(net_env.videochat_time)
                log_file = open(log_path, 'wb')
